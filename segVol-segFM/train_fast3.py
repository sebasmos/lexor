...
#     """Validation routine has been disabled."""
#     pass
# ---------------------------------------------------------
...
def parse_args():
    parser = argparse.ArgumentParser(description='SegVol Training with Fast Encoders (validation disabled)')
...
# Benchmark‑related helpers remain unchanged
...
# Timing‑enabled training remains unchanged
...
def train_one_epoch_with_timing(model, train_dataloader, optimizer, epoch, local_rank, writer,
    ...
    with tqdm(train_dataloader, desc=f'Epoch {epoch}', disable=(local_rank != 0)) as pbar:
        for step, data_item in enumerate(pbar):
            ...
            if log_inference_time and step % 20 == 0:
                torch.cuda.synchronize()
                start_time = time.time()
                loss = model(image=image, train_labels=label[:, 0], train_organs=None)
                torch.cuda.synchronize()
            ...
            if local_rank == 0:
                ...
                if log_inference_time and inference_times:
                    ...
                    writer.add_scalar('train/inference_time_ms', avg_inf,
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if local_rank == 0:
            ...
            if inference_times:
                ...
                writer.add_scalar('train/epoch_avg_inference_time_ms', avg_inf, epoch)
                print(f"Average inference time this epoch: {avg_inf:.2f} ms", flush=True)
            ...
            ckpt = {
                ...
                'loss': avg_loss,
                'encoder_type': fast_encoder_type,
                'config': vars(args),
            }
