...
def validation(model_val, val_dataloader, processor, output_dir):
    ...
    for data_item_list in tqdm(val_dataloader, desc="Validating"):
        ...
        for cls_idx in range(category_num):
            ...
            with torch.no_grad():
                ...
                gt_label = data_item['label'][0][cls_idx].cpu().numpy().astype(np.uint8)
                pred_label = logits_mask[0][0].cpu().numpy().astype(np.uint8)

                bbox_dice = processor.dice_score(torch.from_numpy(pred_label), torch.from_numpy(gt_label), device)
                print(f'cls {cls_idx} - BBox Dice: {bbox_dice.item():.4f}')
                batch_bbox_dice += bbox_dice.item()

                # Save prediction
                base_name = os.path.basename(data_item['file_path'])
                case_name = os.path.splitext(base_name)[0]
                out_path = os.path.join(output_dir, f'{case_name}_cls{cls_idx}_pred.npz')
                np.savez_compressed(out_path, pred=pred_label, gt=gt_label)
...
if __name__ == '__main__':
    ...
    output_dir = './seg_outputs'  # define output directory here
    validation(model_val, val_dataloader, processor, output_dir)
