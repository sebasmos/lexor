        ...
        for case in tqdm(test_cases):
            ...
            for it in range(n_clicks + 1): # + 1 due to bbox pred at iteration 0
                ...
                if torch.cuda.is_available(): # GPU available
                    cmd = 'docker container run --gpus "device=0" -m 32G --name s{} --rm -v $PWD/inputs/:/workspace/inputs/ -v $PWD/outputs/:/workspace/outputs/ {}:latest /bin/bash -c "sh predict.sh" '.format(teamname.replace('/', '_'), teamname.split('_')[0])
                else:
